...
const CourseInformationSheetPage = ({ courseId }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const { formState, importSummary: summary } = res.data;

      // Update React form state with successfully mapped fields
      if (formState.courseSummary) setCourseSummary(formState.courseSummary);
      if (formState.basicInfo) setBasicInfo(formState.basicInfo);
      if (formState.courseObjectives) setCourseObjectives(formState.courseObjectives);
      if (formState.courseContents) setCourseContents(formState.courseContents);
      if (formState.cloData?.length) setCloData(formState.cloData);
      if (formState.textbooks?.length) setTextbooks(formState.textbooks);
      if (formState.obaData?.length) setObaData(formState.obaData);
      if (formState.weeklyPlan?.length) setWeeklyPlan(formState.weeklyPlan);
      if (formState.grading) setGrading(formState.grading);

      setImportSummary(summary);
      toast.success('Form populated from Word document successfully!');
      
      // Smooth scroll to top to see report
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <p style={{ margin: '0.25rem 0 0', fontSize: '0.88rem', color: '#64748b' }}>
              Upload a filled CIS template (.docx) to automatically extract and populate the form fields.
            </p>
          </div>
          <button
            type="button"
